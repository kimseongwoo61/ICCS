...
def change_myinfo(request):
    ...
    if request.method == 'POST':
        ...
        upload_file_save_dir = os.path.join("Google_token/", request.user.username, "vision_token.json")
        fs = FileSystemStorage(location= upload_file_save_dir)
        fs.save(upload_file_save_dir, uploaded_file)
        
        # 업로드된 파일 내용이 json인지 검증한다.
        if is_json(upload_file_save_dir):
            with open(upload_file_save_dir) as f:
                google_vision_token = f.read()
                f.close()
            os.remove(upload_file_save_dir)
